...
# This script is used to connect with aws and it will list the active resources of the service like (EC2, S3, RDS, etc)
...
LOG_FILE="$HOME/aws-inventory.log"
...
add_cron_job() {
    ...
    cron_command="bash $(pwd)/aws_resource_list.sh $region $service >> $LOG_FILE 2>&1"
...
if [[ "$1" == "--schedule" ]]; then
    if [ $# -ne 4 ]; then
        echo "Usage: ./aws_resource_list.sh --schedule \"<cron_time>\" <aws_region> <aws_service>"
        echo "Example: ./aws_resource_list.sh --schedule \"0 9 * * *\" us-east-1 ec2"
        exit 1
    fi
